...
<div id="slideshow">
   <div id="container" style="color:#ccc;background-image: url(/image/1.jpg);background-size:100%;">
	 ...
	 <h2>Price per view: <?php echo $adsprice / 100000000,5;?> BTC</h2>
...
if(isset($_POST["submit"])){
	$NumberOfViews = NumberCheck($_POST["views"]);
	$WebSiteLink = LinkCheck($_POST["link"]);
	if(!empty($NumberOfViews && $WebSiteLink)){
		if($NumberOfViews<61 && $NumberOfViews!=0){
			$link=$WebSiteLink;
			$buyer=$_SESSION["sess_user"];
			$view=$NumberOfViews;
			$COST=$NumberOfViews*$adsprice;
			$result = mysql_query("SELECT *from members WHERE user='" . $buyer . "'");
